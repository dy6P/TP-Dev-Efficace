...
class Dictionnaire:
    ...
    def __setitem__(self, clé, valeur):
        ...
        for i in self.prochain_indice_sondage(h):
            ...
            # Case vide → on insère ici (ou sur une tombstone rencontrée avant)
            if entrée is None:
                if first_tombstone is not None:
                    i = first_tombstone
                self.table[i] = self.Entrée(clé, valeur, h)
                self.nb_elements += 1
                return

            # Tombstone rencontrée : on la garde en mémoire si c’est la première
            elif entrée.supprimé:
                if first_tombstone is None:
                    first_tombstone = i

            # Clé déjà présente : on met à jour la valeur, pas le compteur
            elif entrée.hash == h and entrée.clé == clé:
                entrée.valeur = valeur
                return

        # Si on a quitté la boucle sans insérer, ça veut dire que la table est pleine
        raise RuntimeError("Table de hachage pleine")
    ...
    def __getitem__(self, clé):
        ...
        for i in self.prochain_indice_sondage(h):
            ...
            if entrée is None:
                raise KeyError(clé)
            elif not entrée.supprimé and entrée.hash == h and entrée.clé == clé:
